...
import Ledger (before, from, txInfoValidRange)
import Ledger qualified
import Ledger.Constraints (mustIncludeDatum, mustPayToTheScript, mustSpendScriptOutput, mustValidateIn, otherScript, typedValidatorLookups, unspentOutputs)
...
newtype AddTreasure = AddTreasure
  { _deposit :: Value
  }
  deriving
    ( Playground.Contract.Generic,
...
-- | Redeemer parameters
data ChestRedeemer = ChestRedeemer
...
instance Scripts.ValidatorTypes Morbid where
  type DatumType Morbid = ChestDatum
  type RedeemerType Morbid = ChestRedeemer
...
validate datum redeemer context =
  case _redeemAction redeemer of
    ...
    ActionDelayUnlock ->
      traceIfFalse -- just validate either owner or password
    ...
    ActionUnlockChest ->
      traceIfFalse -- just validate deadline
...
morbidValidator =
  Scripts.mkTypedValidator @Morbid
...
logAs ::
  (AsContractError x) =>
  (Haskell.String -> MorbidContract x ()) ->
  Haskell.String ->
  Haskell.String ->
  MorbidContract x ()
...
getChestDatumFrom ::
  M.Map TxOutRef ChainIndexTxOut ->
  Maybe ChestDatum
getChestDatumFrom =
    ...
    ( do
        ...
        case (getChestDatumFrom utxoS, M.toList utxoS) of
          (Just you, (txOutRef, scriptChainIndexTxOut) : _) -> do
            ...
            now <- currentTime
            let validity =
                  ( unspentOutputs $ txOutRef `M.singleton` scriptChainIndexTxOut
                  )
                    Haskell.<> ( typedValidatorLookups morbidValidator
                               )
                    Haskell.<> ( otherScript contractValidator
                               )
                builtinDatum =
                  Ledger.Datum $
                ...
                txn =
                  ( you `mustPayToTheScript` (_ciTxOutValue scriptChainIndexTxOut + _deposit params)
                  )
        ...
        case (getChestDatumFrom utxoS, M.toList utxoS) of
          (Just d, (txOutRef, scriptChainIndexTxOut) : _) -> do
            ...
            let deadline = now + Haskell.fromInteger (_postponeForSlots params * 1_000)
                ...
                txn =
                  ( you `mustPayToTheScript` (_ciTxOutValue scriptChainIndexTxOut)
                  )
                    <> ( mustSpendScriptOutput txOutRef builtinRedeemer
                       )
                    <> ( mustIncludeDatum builtinDatum
...
type MorbidSchema =
  Endpoint "1. Create Chest" CreateChest
